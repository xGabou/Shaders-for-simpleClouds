...
void main() {
        ...
        discard;
    #else
        // original Complementary rendering
        vec4 color = texture2D(tex, texCoord) * glColor;

        vec4 translucentMult = vec4(
            mix(vec3(0.666), color.rgb * (1.0 - pow2(pow2(color.a))), color.a),
        1.0);

        #ifdef OVERWORLD
            vec3 cloudLight = mix(
                vec3(0.8, 1.6, 1.5) * sqrt1(nightFactor),
                mix(dayDownSkyColor, dayMiddleSkyColor, 0.1),
                sunFactor
            );
