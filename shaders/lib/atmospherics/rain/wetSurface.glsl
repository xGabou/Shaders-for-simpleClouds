    ...
    void ApplyWetSurfaceOverlay(
        ...
        //--------------------------------------------------------------------
        // WETNESS / STORM INTENSITY
        //--------------------------------------------------------------------

        float wetMix;

        #if USE_SC
            // SC version (darkness + thickness + rainfall)
            wetMix = max(GetStormActivity(), GetStormDryingMask());
        #else
            // Vanilla version (rain)
            wetMix = max(rainFactor, wetness);
        #endif
        ...
        #if USE_SC
            stormFactor = 1.0 + 0.4 * clamp(Get_SC_StormDarkness(), 0.0, 1.0);
        #else
            stormFactor = 1.0 + 0.4 * rainFactor;
        #endif
        ...
        float ripplePhase = frameTimeCounter * 0.6
                          + dot(worldPos.xz + cameraPosition.xz, vec2(0.04, 0.07));

        vec2 rippleOffset =
              rippleWind * sin(ripplePhase)
        ...
        vec3 reflectionColor = mix(color, rippleSample, 0.5)
                             + vec3(0.02, 0.025, 0.03) * sunFactor;

        color = mix(color, reflectionColor,
                    wetStrength * (0.35 + 0.35 * fresnel));
    }
